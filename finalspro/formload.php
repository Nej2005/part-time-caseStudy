            ...
            <div class="container">
                <div class="header-content">
                    <div class="logo-container">
                        <button class="back-button" onclick="window.location.href='emp-formload.php'">&larr;</button>
                                                ...
                                                <tr data-detail-id="<?php echo $detail['detail_id']; ?>" <?php echo ($detail['disabled'] ? 'class="row-disabled"' : ''); ?>>
                                                    <td>
                                                        <?php if (isset($_SESSION['edit_mode']) && $_SESSION['edit_mode']) : ?>
                                                            <select name="subject_code[]" class="subject-code" data-row-id="<?php echo $detail['detail_id']; ?>">
                                                                ...
                                                                <?php foreach ($subjects as $subject) : ?>
                                                                    <option value="<?php echo htmlspecialchars($subject['subject_code']); ?>"
                                                                        ...
                                                                        <?php echo (trim($detail['subject_code']) === trim($subject['subject_code'])) ? 'selected' : ''; ?>>
                                                                        <?php echo htmlspecialchars($subject['subject_code']); ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        <?php else : ?>
                                                            <input type="text" name="subject_code[]" class="subject-code"
                                                                value="<?php echo htmlspecialchars($detail['subject_code']); ?>" readonly>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td>
                                                        <?php if (isset($_SESSION['edit_mode']) && $_SESSION['edit_mode']) : ?>
                    ...
                    <div class="bottom-section">
                        ...
                        <table class="bottom-table">
                            <tr>
                                ...
                                <td><input type="number" id="totalLoadUnits" value="<?php echo array_sum(array_column($form_details, 'lec_hours')) + array_sum(array_column($form_details, 'lab_hours')); ?>" class="small-input" readonly></td>
                                </td>
                            </tr>
                            <tr>
...
if (isset($_POST['delete_form'])) {
    ...
    $professor_id = $_GET['professor_id']; // From URL

    // Verify the form exists and belongs to this professor
    $verify_query = "SELECT 1 FROM Form_Loads 
    ...
    try {
        // Just delete from Form_Loads - details will cascade
        ...
        $conn->rollback();
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'Error deleting form: ' . $e->getMessage()]);
        exit();
    }
}
...
function calculateTotalLoadUnits($form_details, $subjects)
{
    ...
    foreach ($form_details as $detail) {
        ...
        if (!in_array($subjectCode, $processedSubjects)) {
            $total += $subjectUnitsMap[$subjectCode] ?? 0;
...
<body>
    ...
    <div class="content-area">
        <header>
            <div class="container">
                <div class="header-content">
                    <div class="logo-container">
                        <button class="back-button" onclick="goBackToEmpFormload()">&larr;</button>
                        <img class="logo" src="Logo.ico" alt="School Logo">
                        <div class="school-name">Faculty Portal</div>
                    </div>
                    <div class="date-time" id="date"></div>
    ...
    <script>
        ...
        window.goBackToEmpFormload = function() {
            console.log('Back button clicked');
            window.location.href = 'emp-formload.php';
        }
        ...
        function initializeEventListeners() {
            ...
            if (backButton) {
                ...
                backButton.removeEventListener('click', goBackToEmpFormload);
                backButton.addEventListener('click', goBackToEmpFormload);
            }
        ...
        function viewSemester(semester) {
            ...
            fetch(`formload.php?professor_id=${<?php echo $professor_id; ?>}&school_year=${year}&semester=${semester}&ajax=1`)
                .then(response => {
        ...
        function initializeEssentialFunctions() {
            ...
            if (backButton) {
                backButton.onclick = goBackToEmpFormload;
            }
        ...
        function convertToEditable() {
            ...
            document.querySelectorAll('input.subject-code').forEach(input => {
                ...
                select.dataset.rowId = input.closest('tr').getAttribute('data-detail-id') || '';
                select.innerHTML = `
                     <option value="" hidden>Select Subject</option>
                    <?php foreach ($subjects as $subject) : ?>
                        <option value="<?php echo htmlspecialchars($subject['subject_code']); ?>"
                            data-description="<?php echo htmlspecialchars($subject['subject_name']); ?>"
                            ${input.value === '<?php echo htmlspecialchars($subject['subject_code']); ?>' ? 'selected' : ''}>
                            <?php echo htmlspecialchars($subject['subject_code']); ?>
                        </option>
                    <?php endforeach; ?>
                `;
            ...
            document.querySelectorAll('.subject-description-combined').forEach(combinedInput => {
                ...
                let baseDescription = combinedValue.replace(/\s*\(LEC\)|\s*\(LAB\)|\s*\(\)/, '').trim();
                let type = '';

                if (combinedValue.includes('(LEC)')) {
                    type = 'LEC';
                } else if (combinedValue.includes('(LAB)')) {
                    type = 'LAB';
                }
        ...
        function updateSubjectType(select) {
            ...
            if (descriptionInput && descriptionInput.value) {
                if (combinedInput) {
                    combinedInput.value = descriptionInput.value + (type ? ` (${type})` : '');
                }
            }
        ...
        function calculateTotals() {
            ...
            if (totalLoadUnitsElement) {
                totalLoadUnitsElement.value = totalLec + totalLab;
            }
        ...
        function addNewRow() {
            ...
            selects.forEach(select => {
                if (select.name === 'subject_code[]') {
                    ...
                    select.selectedIndex = 0;
                } else if (select.name === 'subject_type[]') {
                    select.selectedIndex = 0;
                }
            });
            ...
            fetch('../Backend/save_formload.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Instead of reloading the page, we'll refresh the content with the current semester/year
                        const urlParams = new URLSearchParams(window.location.search);
        ...
        function confirmDelete() {
            if (confirm('Are you sure you want to delete this loading form?')) {
                ...
                fetch('../Backend/delete_form.php', {
                        ...
                        body: `form_id=${formId}&professor_id=${professorId}`
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            contentArea.innerHTML = '<div class="loading" style="color: green;">' + data.message + '... redirecting you to Form Loading page.</div>';
                            setTimeout(() => {
        ...
        function sendToProfessor() {
            if (confirm('Are you sure you want to send this loading form to the professor?')) {
                // AJAX call to send the form
                fetch('../Backend/send_to_professor.php', {
                        method: 'POST',
                        body: JSON.stringify({
                            form_id: <?php echo json_encode(($form_to_display ? $form_to_display['form_id'] : ($current_form ? $current_form['form_id'] : null))); ?>,
                            professor_id: <?php echo json_encode($professor_id); ?>
                        }),
                        headers: {
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        alert('Error sending form to professor');
                    });
            }
        }
        ...
        function exportForm() {
            ...
            const subjectData = rows.map(row => {
                return {
                    ...
                    lec: row.querySelector('input[name="lec_hours[]"]').value,
                    lab: row.querySelector('input[name="lab_hours[]"]').value,
                    hrsPerWeek: row.querySelector('input[name="hrs_per_week[]"]').value,
            ...
            fetch('../Backend/Exporting/export_formload.php', {
                ...
                .then(blob => {
                    ...
                    a.href = url;
                    a.download = `Faculty_Loading_${name.replace(/\s+/g, '_')}_${semesterSubtitle.replace(/\s+/g, '_')}.xlsx`;

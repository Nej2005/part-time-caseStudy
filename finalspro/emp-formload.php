...
<head>
    ...
    <style>
        * {
        ...
        #noRecordsModal .modal-content {
            text-align: center;
        }

        #noRecordsModal .modal-body {
            padding: 20px;
            font-size: 16px;
        }

        #noRecordsModal .modal-footer {
            justify-content: center;
        }
...
<body>
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div class="sidebar-menu">
                <ul>
                    ...
                    <li><a href="" class="active"><i class="fas fa-calendar-alt"></i> Form Loading</a></li>
                    ...
                    <li><a href="about.php" id="bot-sidebar"><i class="fas fa-info-circle"></i> About</a></li>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <button class="btn btn-primary"
                                            onclick="document.location='createnew-forml.php?professor_id=<?php echo $professor['professor_id']; ?>'">
                                            <i class="fas fa-plus"></i> Create New
                                        </button>
                                        <button class="btn btn-secondary"
                                            onclick="viewRecords(<?php echo $professor['professor_id']; ?>)">
    ...
    <script>
        ...
        function viewRecords(professorId) {
            window.location.href = `formload.php?professor_id=${professorId}`;
        }
        ...
        saveEmployeeBtn.addEventListener("click", function() {
            ...
            if (isUpdate) {
               ...
               <td>${fullName}</td>
                <td>
                  <select class="progress-dropdown pending"><option value="completed">Completed</option>  
               ...
               <td>
                  <div class="action-buttons">
                    <button class="btn btn-primary" onclick="openCreateNewModal()">
                        <i class="fas fa-plus"></i> Create New
                    </button>
                    <button class="btn btn-secondary" onclick="viewPreviousRecords(this)">
                        <i class="fas fa-file-alt"></i>  View Previous Records
                    </button>
                  </div>
               </td>
        ...
        function viewPreviousRecords(button) {
            const row = button.closest("tr");
            const employeeName = row.cells[1].innerText;
            alert(`Viewing previous records for ${employeeName}`);
        }
        ...
        function viewRecords(professorId) {
            fetch(`../Backend/check_form_records.php?professor_id=${professorId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.hasRecords) {
                        window.location.href = `formload.php?professor_id=${professorId}`;
                    } else {
                        // Show the no records modal
    ...
    <!-- No Records Modal -->
    <div class="modal" id="noRecordsModal">
        <div class="modal-content" style="width: 400px;">
            <div class="modal-header">
                <h2><i class="fas fa-exclamation-circle"></i> No Records Found</h2>
                <button class="close-modal" onclick="document.getElementById('noRecordsModal').style.display = 'none'">×</button>
            </div>
            <div class="modal-body">
                <p>No data has been created for this professor.</p>
            </div>
            <div class="modal-footer">
                <button class="btn btn-cancel" onclick="document.getElementById('noRecordsModal').style.display = 'none'">Cancel</button>
            </div>
        </div>
    </div>
</body>
